...
if (!empty($post_images)) {
    $arr = $post_images->post_images;
    $arr_images = json_decode($post_images->post_images);
}
?>
<!-- POST IMAGE -->
<div class="form-group config-images">
    {!! Form::label('image',trans('posts.post_images'),': *') !!}
    {!! Form::file('image') !!}
    {!! Form::hidden('is_file', !empty(@$post->post_image)?1:0, ['class' => 'is_file']) !!}
...
@if(@$post->post_image)
<div class="img-thumb">
    <img src="{!! url($data['configs']['post']['urlpath'].'/'.$post->post_image) !!}">
</div>
...
<table class="table table-hover images-table">
    ...
    <tbody>
        ...
        <tr class="item_image">
            ...
            <td>
                <div class="img-thumb">
                    <img src="{!! url($data['configs']['post']['urlpath'].'/'.$item->name) !!}">
